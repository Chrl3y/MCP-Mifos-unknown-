...
export const listWorkflowRunsConfig = {
    ...
    inputSchema: z.object({
        owner: z.string().describe('Repository owner.'),
        repo: z.string().describe('Repository name.'),
        workflow_id: z.union([z.string(), z.number()]).optional().describe('Filter by workflow ID or file name.'),
        status: z.enum(['queued', 'in_progress', 'completed', 'waiting', 'requested', 'pending']).optional().describe('Filter by status.'),
    }),
};
...
export type TriggerWorkflowDispatchInput = z.infer<typeof triggerWorkflowDispatchConfig.inputSchema>;
